...
-- Ashita 4.3 rebuilt the key item pointer for the current retail client, which
-- leaves GetPlayer():HasKeyItem() returning false on the HorizonXI client build.
-- Track the Clamming kit from chat text until the API starts working again.
local kiApiWorking = false;
local hasClammingKit = function(clammy)
	local ok, has = pcall(function()
		return AshitaCore:GetMemoryManager():GetPlayer():HasKeyItem(511);
	end);
	if (ok == true and has == true) then
		kiApiWorking = true;
		return true;
	end
	if (kiApiWorking == true) then
		return false;
	end
	return (clammy ~= nil and clammy.kitFallback == true);
end
...
local handleBucket = function(clammy)
	-- The bucket broke. Everything in it went back into the sea, so clear it
	-- out and log it as lost instead of leaving the final dig frozen on
	-- screen until a new kit happens to be bought.
	if (clammy.pendingBreak == true) then
		clammy.pendingBreak = false;
	...
	-- A turn-in we only learned about second hand, from the new kit rather
	-- than from the message we missed.
	if (clammy.pendingTurnIn == true) then
		clammy.pendingTurnIn = false;
	...
	if (hasBucketKI == true) then
		...
		if (clammy.hasBucket == true) and (clammy.bucketIsBroke == true) then
			...
			clammy.playTone = true;
			clammy.stopSound = true;
			-- Without this the flags settle on hasBucket=false plus
			-- bucketIsBroke=true, which matches no branch at all and leaves
			-- the meter stuck at 0% until the next dig.
			clammy.bucketIsBroke = false;
			return clammy;
		end
		return clammy;
...
func.handleChatCommands = function(args, clammy)
	...
	if (#args == 2 and args[2]:any('turnin')) then
		clammy.kitFallback = false;
		clammy = func.emptyBucket(clammy, true, false);
		clammy = calculateTimePerBucket(clammy);
		clammy = updateLastClammingAction(clammy);
		print(chat.header(addon.name):append(chat.message('Bucket turned in.')));
		return clammy;
	end
	...
	if (args[2]:any('debug')) then
		if (args[3] == nil) then
			print(chat.header(addon.name):append(chat.message('debug subcommands: ki, setbucketsize <n>, breakbucket')));
			return clammy;
		end
		if (args[3]:any('ki')) then
			local p = AshitaCore:GetMemoryManager():GetPlayer();
			local areaId = AshitaCore:GetMemoryManager():GetParty():GetMemberZone(0);
			local ok511, has511 = pcall(function() return p:HasKeyItem(511); end);
			print(chat.header(addon.name):append(chat.message(('zone=%s   HasKeyItem(511)=%s   apiOk=%s'):fmt(tostring(areaId), tostring(has511), tostring(ok511)))));
			local total = 0;
			local nearby = {};
			for i = 0, 2047 do
				local ok, has = pcall(function() return p:HasKeyItem(i); end);
				if (ok and has == true) then
					total = total + 1;
					if (i >= 400 and i <= 700) then table.insert(nearby, tostring(i)); end
				end
			end
			print(chat.header(addon.name):append(chat.message(('total key items detected: %d'):fmt(total))));
			print(chat.header(addon.name):append(chat.message('ids 400-700: ' .. table.concat(nearby, ', '))));
			return clammy;
		end
...
func.handleTextIn = function(e, clammy)
	...
	-- breaking a bucket all have to register even if you have already run
	-- off toward the boat before the text finishes arriving.
	if (string.match(lowerMessage, "key item") ~= nil) and (string.match(lowerMessage, "clamming kit") ~= nil) then
		-- A fresh kit while we still believe a bucket is in hand means the
		-- turn-in message never reached us. Close the old bucket out so its
		-- contents do not carry over into the new one.
		if (clammy.hasBucket == true) and (clammy.bucketIsBroke == false) then
			clammy.pendingTurnIn = true;
		end
		clammy.kitFallback = true;
		hasBucketKI = true;
	end
	if (string.match(lowerMessage, "toss it into your bucket") ~= nil) then
		clammy.kitFallback = true;
		hasBucketKI = true;
	end
	if (string.match(lowerMessage, "you return the") ~= nil) and (string.match(lowerMessage, "clamming kit") ~= nil) then
		clammy.kitFallback = false;
	end
	if (string.match(lowerMessage, "washed back into the sea") ~= nil) then
		-- Deferred rather than handled here, so it always runs after the
		-- "you find a ..." line from the same dig has been added to the
		-- bucket - whichever order the two messages arrive in.
		clammy.pendingBreak = true;
		clammy.kitFallback  = false;
	end
	...
	if (string.match(e.message, "You find a") and (hasBucketKI == true)) then
		for idx,citem in ipairs(clammy.items) do
			if (string.match(string.lower(e.message), string.lower(citem.item)) ~= nil) then
				...
				-- Must be >=, not >. The bar tracks the final 50 ponzes, and
				-- at exactly 50 remaining the modulo below yields 0, which
				-- reads as an empty bucket rather than a full segment. That
				-- is every multiple of 50 - exactly where an upgrade lands.
...
func.renderClammy = function(clammy)
	...
	if (imgui.Begin('Clammy', true, bit.bor(ImGuiWindowFlags_NoDecoration))) then
		...
		imgui.PushFont(nil, baseFontSize * normalFontSize);
		if (clammy.hasBucket == true) then
		...
		imgui.SameLine();
		imgui.PushFont(nil, baseFontSize * enlargedFontSize);
		...
		if (Config.showPercentChanceToBreak[1] == true) then
			...
			imgui.PushFont(nil, baseFontSize * enlargedFontSize); imgui.SetCursorPosY(imgui.GetCursorPosY() - (2 * Config.windowScaling[1]));
			imgui.TextColored(bucketBreakChance.color, formatChanceBreak(bucketBreakChance.percentWeight)); imgui.SameLine();
			imgui.PopFont(); imgui.SetCursorPosY(imgui.GetCursorPosY() + (2 * Config.windowScaling[1]));
